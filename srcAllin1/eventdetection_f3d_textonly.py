...
def statisticDF(dataFilePath, predefinedUnitHash):
    global twHash
    twContent = file("../ni_data/tweet_id_tw.txt").readlines()
    twContent = [line.strip().split(" ")[1] for line in twContent]
    twHash = dict(zip(range(len(twContent)), twContent))
...
############################
## represent text string into tf-idf vector
def toTFIDFVector(text, wordDFHash):
...
def calpairsim(segList, segTextHash, wordDFHash):
    segPairHash = {} #segId1|segId2:sim
    segVecNormHash = {} # segId:seg vec norm
    segTVecHash = {} # segId:seg's tfidf vec
    segNum = len(segList)
    for segId, seg in enumerate(segList):
        segContextText = segTextHash[segId]
        if segContextText.endswith("###"):
            segContextText = segContextText[:-3]

        [featureHash, norm] = toTFIDFVector(segContextText, wordDFHash)
    ...
    for i in range(0,segNum):
        for j in range(i+1,segNum):
            segPair = str(i) + "|" + str(j)
            tSim = textSim(segTVecHash[i], segVecNormHash[i], segTVecHash[j], segVecNormHash[j])
            segPairHash[segPair] = tSim
    return segPairHash 
...
def calsegText(segList, unitHash, unitInvolvedHash, tStr):
    ...
    for segId, seg in enumerate(segList):
        seg_rel_tweets = [involvedTweets[tid] for tid in unitHash[seg[2]][tStr]]
        segTextHash[segId] = "###".join(seg_rel_tweets)
    return segTextHash
...
def calWordDF():
    ...
    for lineid, line in enumerate(content):
        ...
        for word in arr:
            ...
            word_df_hash[lineid]=1
            wordDFHash[word] = word_df_hash
    for word in wordDFHash:
        wordDFHash[word] = len(wordDFHash[word])
    return wordDFHash
...
def detection(unitHash, windowHash, unitpsHash, wordDFHash, kNeib, taoRatio):
    ...
    for tStr in sorted(windowHash.keys()):
        ...
        segTextHash = calsegText(segList, unitHash, unitInvolvedHash, tStr)
        segPairHash = calpairsim(segList, segTextHash, wordDFHash)
