...
def getRelskl_fromPOS(filename, chunkedFilename, outputFilename):
...
#            print resultArr
            ...
            #outputFile.write(str(lineIdx-1) + "\t" + " ".join(resultArr) + "\n")
...
#                        lvHash[tmpWord] = 1
        ...
        # triple structure
        if len(relArr[0])+len(relArr[2])==0: continue
        resultArr.append("|".join(relArr))
...
if __name__ == "__main__":
    ...
    print "Usage getRelSkl.py posFileName chunkedFileName outputFilename (frame_tStr) [lex] "
    filename = sys.argv[1]
    chunkedFilename = sys.argv[2]
    outputFilename = sys.argv[3]
    getRelskl_fromPOS(filename, chunkedFilename, outputFilename) # extract skl from Postagged file(self-completed reverb)
