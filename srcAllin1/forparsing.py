...
def loadDataFromFiles(filepath, outfilepath):
    lineIdx = 0
    print "### " + str(time.asctime()) + " Reading from file " + ": " + filepath
    subFile = file(filepath)
    newTagtextFile = file(outfilepath, "w")
    while True:
...
dirPath = currDir
print "Usage: python forparsing.py inputfilename outfilename\n"
print "Program starts at time:" + str(time.asctime())

loadDataFromFiles(sys.argv[1], sys.argv[2])
