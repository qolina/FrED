...
############################
## load seg pair
def loadsklPair(filepath):
    inFile = file(filepath,"r")
    sklContentHash = cPickle.load(inFile)
    sklPairHash = cPickle.load(inFile)
    inFile.close()
    return sklContentHash , sklPairHash
...
def clusterEventSegment(dataFilePath, kNeib, taoRatio):
    ...
    for item in sorted(fileList):
        ...
        sklPairFilePath = dataFilePath + "sklPairFile_right" + tStr
        [sklContentHash, sklPairHash] = loadsklPair(sklPairFilePath)

        print "### " + str(time.asctime()) + " " + str(len(sklContentHash)) + " event skeletons in " + sklPairFilePath  + " are loaded. With skeleton pairs Num: " + str(len(sklPairHash))

        # get segments' k nearest neighbor
        kNNHash = {}
        for pair in sklPairHash:
            sim = sklPairHash[pair]
            segArr = pair.split("|")
            #segId1 = int(segArr[0])
            #segId2 = int(segArr[1])
            segId1 = segArr[0]
        ...
        print "### " + str(time.asctime()) + " " + str(len(kNNHash)) + " event skeletons' " + str(kNeib) + " neighbors are got."
 ...
 #               print kNNHash[segId2]
                ...
                if segId1 in kNNHash[segId2]:
                    ...
                    edgeHash[edge] = sklPairHash[edge]
  ...
  #              validEventHash[eventId] = mu_max/mu_e
        ...
        for eventID in eventHash:
            ...
            for node in nodeHash:
                relSet = sklContentHash[node].relSet
                simpleRels = []
                for rel in relSet:
                    warr = rel.split("||")
                    warrNew = []
                ...
                #print "\t".join(simpleRels)
                eventFile.write("Skl" + str(sklId) + ": " + "\t".join(simpleRels) + "\n")
                sklId += 1
            #print "\n"
            eventFile.write("\n")
